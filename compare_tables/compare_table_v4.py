...
def preprocess_scan_list(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df_processed.iterrows():
        ...
        if comma_count == 2:
            ...
            df_processed.loc[idx, "箱号"] = cabinet_no.strip()
            df_processed.loc[idx, "渠道号"] = channel_no.strip()
            df_processed.loc[idx, "托盘号"] = pallet_no.strip()
        # 如果不符合格式，根据第4列是否为空采用不同处理
        if comma_count < 2:
            # 判断第4列是否为空
            col4_empty = pd.isna(row[col4]) or str(row[col4]).strip() == ""
            
            if col4_empty:
                # 第4列为空，可能误扫了条码到托盘贴位置
                # 原始行保持原值
                df_processed.loc[idx, "箱号"] = val.strip()
                
                # 创建修正行：把第2列移到第4列
                new_row = row.copy()
                new_row[col4] = row[col2]
                if val.strip() == "":
                    new_row["箱号"] = '作为托盘贴被扫描：空白'
                else:
                    new_row["箱号"] = f'作为托盘贴被扫描：{val.strip()}'
                extra_rows.append(new_row)
            else:
                # 第4列有条码，只是托盘贴格式错误
                # 原始行标记为"托盘贴错误"，不创建修正行
                if val.strip() == "":
                    df_processed.loc[idx, "箱号"] = '托盘贴错误：空白'
                else:
                    df_processed.loc[idx, "箱号"] = f'托盘贴错误：{val.strip()}'
...
def preprocess_pkg_list(filename: str) -> tuple[pd.DataFrame, list[str]]:
    """预处理"包裹清单"sheet，将分组列展开为统一五列
    
    Returns:
        tuple: (DataFrame, 渠道列表)
            - DataFrame: 处理后的包裹清单数据
            - list[str]: 预处理后的有效渠道列表（已去除CWE-和卡派-前缀）
    """
    ...
    for idx_pre, idx_tuo, idx_ref, idx_bad in zip(col_pre, col_tuo, col_ref, col_bad):
        ...
        # 收集有效渠道
        if channel != 'nan' and channel.strip():
    ...
    return combined_df, channels_list
...
def compare_tables(df_a: pd.DataFrame, df_b: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in res.iterrows():
        ...
        for _, match in match_rows.iterrows():
            ...
            same_box = pd.notna(scan_box) and pd.notna(b_box) and str(scan_box).lower() == str(b_box).lower()
            same_channel = pd.notna(scan_channel) and pd.notna(b_channel) and str(scan_channel).lower() == str(b_channel).lower()
...
def export_backfill_to_original(original_file: str, compared_df: pd.DataFrame, output_filename: str, unreported_df: pd.DataFrame = None):
    """
    导出回填结果 excel
    """
    ...
    for idx, row in compared_df.iterrows():
        ...
        # 判断是否应该回填：只有黄色（条码匹配且箱号或渠道不对齐）才回填
        code_match = str(row.get('条码匹配', '')).strip()
        box_align = str(row.get('箱号对齐', '')).strip()
        channel_align = str(row.get('渠道对齐', '')).strip()
        
        # 只在黄色情况下回填破损/不可识别列
        if code_match == '是' and (box_align == '否' or channel_align == '否'):
            ...
            is_error_format = (
                box_str and 
                ("托盘贴错误：" in box_str or "作为托盘贴被扫描：" in box_str)
            )
            
            damaged_value = None
            if is_error_format:
                # 情况4-5：托盘贴格式错误，保持现有逻辑（显示错误标记）
                ...
                # 情况1-3：托盘贴格式正常，拼接完整托盘贴（箱号,渠道号,托盘号）
                # 箱号：优先用扫描箱号，否则用包裹清单箱号
                final_box = box_str if box_str else str(row.get('箱号', ''))
                # 渠道号：优先用扫描渠道号，否则用包裹清单渠道号
                final_channel = channel_str if channel_str else str(row.get('渠道号', ''))
                # 托盘号：用扫描托盘号
                scan_pallet = row.get('扫描托盘号', '')
                final_pallet = str(scan_pallet) if pd.notna(scan_pallet) else ''
                
                # 拼接三部分
                parts = [final_box, final_channel, final_pallet]
                parts = [p.strip() for p in parts if p and str(p).strip() and str(p).strip() != 'nan']
                damaged_value = ','.join(parts) if parts else None
    ...
    # 处理未预报数据回填
    if unreported_df is not None and len(unreported_df) > 0:
        backfill_unreported_to_workbook(wb, ws, compared_df, unreported_df, original_file)
    
    wb.save(output_filename)
    print(f"已回填到 {output_filename}")


def extract_channel_info_from_excel(filename: str) -> dict:
    """
    从原始Excel文件中提取渠道的列位置信息
    
    Args:
        filename: Excel文件路径
        
    Returns:
        dict: {渠道名: {'col_scan': 列号, 'col_damaged': 列号, 'last_row': 行号}}
    """
    raw_pkg = pd.read_excel(filename, sheet_name="包裹清单", header=None)
    row1 = raw_pkg.iloc[0]
    row2 = raw_pkg.iloc[1]
    ...
    for idx_pre in col_pre:
        ...
        # 跳过无效渠道
        if channel == 'nan' or not channel.strip():
            continue
        ...
        for offset in range(10):
            ...
            if check_idx < len(row2):
                ...
                if pd.notna(header):
                    if "实际扫描" in str(header) and scan_col is None:
                        scan_col = check_idx + 1  # Excel列号从1开始
                    elif ("破损" in str(header) or "不可识别" in str(header)) and damaged_col is None:
                        damaged_col = check_idx + 1
        
        # 查找该渠道的最后一行（含数据的行）
        last_row = 2  # 默认从第3行开始（Excel行号）
        data_col = raw_pkg.iloc[2:, idx_pre]
        non_empty_rows = data_col[data_col.notna()].index.tolist()
        if non_empty_rows:
            last_row = non_empty_rows[-1] + 1  # 转换为Excel行号
        
        if scan_col and damaged_col:
            channel_info[channel] = {
                'col_scan': scan_col,
                'col_damaged': damaged_col,
                'last_row': last_row
            }
...
def backfill_unreported_to_workbook(wb, ws, compared_df: pd.DataFrame, unreported_df: pd.DataFrame, original_file: str = None):
    """
    将未预报数据回填到Excel工作簿
    ...
    Args:
        wb: 工作簿对象
        ws: 工作表对象
        compared_df: 比较结果DataFrame
        unreported_df: 未预报结果DataFrame
        original_file: 原始包裹清单文件路径（当compared_df为空时需要）
    """
    purple_fill = PatternFill(start_color="BF40BF", end_color="BF40BF", fill_type="solid")
    
    # 筛选可回填和异常数据
    backfillable = unreported_df[(unreported_df['是否匹配'] == '否') & (unreported_df['可否回填'] == '是')].copy()
    exceptions = unreported_df[(unreported_df['是否匹配'] == '否') & (unreported_df['可否回填'] == '否')].copy()
    
    if len(backfillable) == 0 and len(exceptions) == 0:
        return
    
    # 从compared_df中提取每个渠道的列位置信息
    channel_info = {}
    for _, row in compared_df.iterrows():
        ...
        if channel not in channel_info:
            channel_info[channel] = {
                'col_scan': int(row.get('_excel_col_scan')) if pd.notna(row.get('_excel_col_scan')) else None,
                'col_damaged': int(row.get('_excel_col_damaged')) if pd.notna(row.get('_excel_col_damaged')) else None,
                'last_row': int(row.get('_excel_row')) if pd.notna(row.get('_excel_row')) else 2
            }
        else:
            # 更新最后一行
            excel_row = int(row.get('_excel_row')) if pd.notna(row.get('_excel_row')) else 2
            if excel_row > channel_info[channel]['last_row']:
                channel_info[channel]['last_row'] = excel_row
    
    # 如果channel_info为空（compared_df为空），从原始文件中提取列位置信息
    if len(channel_info) == 0 and original_file:
        print(f"  检测到比较结果为空，从原始文件提取渠道列位置信息...")
        channel_info = extract_channel_info_from_excel(original_file)
        print(f"  提取到 {len(channel_info)} 个渠道的列位置信息")
    
    # 回填未预报数据
    backfilled_count = 0
    if len(backfillable) > 0:
        # 按渠道分组
        for channel, group in backfillable.groupby('渠道号'):
            channel = str(channel).strip() if pd.notna(channel) else ''
            if not channel or channel not in channel_info:
                continue
            
            info = channel_info[channel]
            col_scan = info['col_scan']
            col_damaged = info['col_damaged']
            start_row = info['last_row'] + 1
            
            if col_scan is None or col_damaged is None:
                continue
            ...
            for i, (_, rec) in enumerate(group.iterrows()):
                ...
                # 填写实际扫描列：原始条码
                scan_code = rec.get('条码', '')
                if pd.notna(scan_code) and scan_code:
                    ws.cell(row=target_row, column=col_scan).value = str(scan_code)
                    ws.cell(row=target_row, column=col_scan).fill = purple_fill
                
                # 填写破损/不可识别列：未预报
                ws.cell(row=target_row, column=col_damaged).value = "未预报"
                ws.cell(row=target_row, column=col_damaged).fill = purple_fill
                
                backfilled_count += 1
        
        print(f"  未预报回填: 已回填 {backfilled_count} 条未预报数据")
    
    # 创建异常Sheet
    if len(exceptions) > 0:
        if "异常" in wb.sheetnames:
            del wb["异常"]
        
        ws_exception = wb.create_sheet("异常")
        
        # 准备导出列
        scan_cols = [c for c in exceptions.columns if c not in ["是否匹配", "可否回填", "预报单号", "托盘序号", "操作箱号", "操作渠道号", "_excel_row"]]
        if 'fba条码' in scan_cols:
            scan_cols.remove('fba条码')
            scan_cols.insert(scan_cols.index('条码') + 1 if '条码' in scan_cols else 0, 'fba条码')
        
        new_cols = ["是否匹配", "可否回填", "预报单号", "托盘序号", "操作箱号", "操作渠道号"]
        final_cols = scan_cols + new_cols
        final_cols = [c for c in final_cols if c in exceptions.columns]
        
        export_df = exceptions[final_cols].copy()
        export_df = export_df.reset_index(drop=True)
        
        # 写入标题行
        for col_idx, col_name in enumerate(final_cols, 1):
            ws_exception.cell(row=1, column=col_idx).value = col_name
        
        # 写入数据
        for row_idx, (_, row) in enumerate(export_df.iterrows(), 2):
            for col_idx, col_name in enumerate(final_cols, 1):
                ws_exception.cell(row=row_idx, column=col_idx).value = row[col_name]
        
        print(f"  异常数据: 已创建异常Sheet，包含 {len(exceptions)} 条记录")
...
def export_unreport_with_colors(df: pd.DataFrame, filename: str):
    ...
    导出未预报结果
    """
    scan_cols = [c for c in df.columns if c not in ["是否匹配", "可否回填", "预报单号", "托盘序号", "操作箱号", "操作渠道号", "_excel_row"]]
    if 'fba条码' in scan_cols:
        scan_cols.remove('fba条码')
        scan_cols.insert(scan_cols.index('条码') + 1 if '条码' in scan_cols else 0, 'fba条码')
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        for idx, row in export_df.iterrows():
            ...
            if is_matched == "否":
                ...
                box_match = (scan_box.lower() == op_box.lower()) or (not scan_box and not op_box)
                channel_match = (scan_channel.lower() == op_channel.lower()) or (not scan_channel and not op_channel)
...
def filter_valid_boxes(df: pd.DataFrame, filename: str) -> pd.DataFrame:
    """
    筛选逻辑：只保留箱号匹配文件名的记录
    """
    if df.empty or "箱号" not in df.columns:
        return df
    
    # 从文件名提取箱号
    cont_name = Path(filename).stem
    box_name = cont_name.split("操作分表")[0].strip()
    box_name_lower = box_name.lower()  # 用于大小写不敏感比较
    
    def is_valid_box(box_value):
        if pd.isna(box_value):
            return False
        box_str = str(box_value).strip()
        box_str_lower = box_str.lower()
        
        # 1. 完全匹配（忽略大小写）
        if box_str_lower == box_name_lower:
            return True
        
        # 2. 以box_name-开头（忽略大小写）
        if box_str_lower.startswith(box_name_lower + "-"):
            return True
        
        # 3. 托盘贴错误
        if box_str.startswith("托盘贴错误："):
            error_box = box_str.replace("托盘贴错误：", "").strip().lower()
            if error_box == box_name_lower or error_box.startswith(box_name_lower + "-"):
                return True
        
        return False
    
    filtered_df = df[df["箱号"].apply(is_valid_box)].copy()
    
    original_count = len(df)
    filtered_count = len(filtered_df)
    dropped_count = original_count - filtered_count
    if dropped_count > 0:
        print(f"筛选完成: 原数据 {original_count} 行 -> 筛选后 {filtered_count} 行 (过滤了 {dropped_count} 行)")
    else:
...
def export_merged_with_colors(df_compare: pd.DataFrame, df_unreport: pd.DataFrame, filename: str):
    ...
    # --- Sheet 2 ---
    scan_cols = [c for c in df_unreport.columns if c not in ["是否匹配", "可否回填", "预报单号", "托盘序号", "操作箱号", "操作渠道号", "_excel_row"]]
    if 'fba条码' in scan_cols:
        scan_cols.remove('fba条码')
        scan_cols.insert(scan_cols.index('条码') + 1 if '条码' in scan_cols else 0, 'fba条码')
    new_cols = ["是否匹配", "可否回填", "预报单号", "托盘序号", "操作箱号", "操作渠道号"]
...
def process_full_workflow(table_b_path: str, preprocessed_scan_df) -> bool:
    ...
    try:
        raw_pkg2, valid_channels_from_pkg = preprocess_pkg_list(table_b_path)
        print(f"  预处理完成，共 {len(raw_pkg2)} 行数据")
        print(f"  提取到 {len(valid_channels_from_pkg)} 个渠道: {valid_channels_from_pkg}")
    except Exception as e:
        print(f"  错误: 预处理失败 - {e}")
        return False
    ...
    # 筛选逆向结果
    df_unreport_filtered = filter_valid_boxes(df_unreport, table_b_path)
    print(f"  逆向结果筛选完毕: {len(df_unreport)} -> {len(df_unreport_filtered)} 行")
    
    # 添加"可否回填"列：基于渠道号判断
    # 如果比较结果为空（包裹清单无预报单号数据），则使用从包裹清单提取的渠道列表
    if len(df_compare) == 0 or df_compare.empty:
        valid_channels = set(valid_channels_from_pkg)
        print(f"  使用包裹清单渠道列表（比较结果为空）: {valid_channels}")
    else:
        valid_channels = set(df_compare['渠道号'].dropna().unique())
        print(f"  使用比较结果渠道列表: {valid_channels}")
    
    def check_backfillable(row):
        channel = row.get('渠道号')
        if pd.isna(channel) or str(channel).strip() == '':
            return '否'
        return '是' if str(channel).strip() in valid_channels else '否'
    
    df_unreport_filtered['可否回填'] = df_unreport_filtered.apply(check_backfillable, axis=1)
    
    # 统计可否回填情况
    backfillable_count = (df_unreport_filtered['可否回填'] == '是').sum()
    not_backfillable_count = (df_unreport_filtered['可否回填'] == '否').sum()
    print(f"  可否回填统计: 可回填 {backfillable_count} 行, 不可回填 {not_backfillable_count} 行")
    ...
    try:
        export_backfill_to_original(str(table_b_path_obj), df_compare, str(backfill_file), df_unreport_filtered)
    except Exception as e:
        print(f"  错误: 回填导出失败 - {e}")
