...
def export_backfill_to_original(original_file: str, compared_df: pd.DataFrame, output_filename: str, unreported_df: pd.DataFrame = None):
    ...
    if unreported_df is not None and len(unreported_df) > 0:
        backfill_unreported_to_workbook(wb, ws, compared_df, unreported_df)
    
    wb.save(output_filename)
    print(f"已回填到 {output_filename}")


def backfill_unreported_to_workbook(wb, ws, compared_df: pd.DataFrame, unreported_df: pd.DataFrame):
    """
    将未预报数据回填到Excel工作簿
    """
